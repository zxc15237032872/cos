...
def calculate_spoke_number8(mask):
    """
    计算图像的辐条数
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        u_times =  max(resultA, resultB) // 5
        c_times = max(resultA, resultB) if max(resultA, resultB) < 10 else 10
        ...
        if similarity < 0.69 and min(first_angleA, first_angleB) > 0.0:
            # 使用较小的角度重新处理
        ...
        if similarity < 0.69:
            ...
            for angle in np.arange(current_best_angle - 0.95, current_best_angle + 1, 0.05):  # 以 0.05 度为步长
                ...
                if similarity < current_similarity:
                    similarity = current_similarity
        resultA = calculate_spoke_number5(clustering1)
        print(f"第一张图像的辐条数：{resultA}")
        resultB = calculate_spoke_number5(clustering2)
        print(f"第二张图像的辐条数：{resultB}")
...
for pair in image_pairs:
    ...
    with open('xie5output.txt', 'a', encoding='utf-8') as file:
        file.write(
