...
def calculate_spoke_number5(mask):
    ...
    if len(all_maxima_indices) > 0:
        ...
        max_maxima_score = template_scores_smooth[max_maxima_index]
        max_maxima_angle = angles[max_maxima_index]

        # 筛选出与最大的极大值相差不到 0.02 的相似度的极大值
        valid_maxima_indices = []
        for index in all_maxima_indices:
            score = template_scores_smooth[index]
            angle = angles[index]
            if abs(score - max_maxima_score) < 0.02 and angle < max_maxima_angle:
                valid_maxima_indices.append(index)

        if len(valid_maxima_indices) > 0:
            # 找到符合条件的极大值中最小的角度对应的索引
            min_angle_index = valid_maxima_indices[np.argmin(angles[valid_maxima_indices])]
            corresponding_angle = angles[min_angle_index]
            print(f"最佳匹配角度为 {corresponding_angle}")
        else:
            # 没有符合条件的极大值，采用最大相似度的最大值对应的角度
            corresponding_angle = max_maxima_angle
            print(f"最佳匹配角度为 {corresponding_angle}")
...
def process_image_after_clustering1(image, times, first_angle):
    """
    对聚类后的掩码图像进行处理，包括旋转和交集操作。
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        size = size if size < 120 else 120
        ...
        u_times = 3 if min(resultA, resultB) <= 10 else 10
        c_times = 6 if min(resultA, resultB) <= 10 else 10



        print(u_times, c_times)

        mask1 = cv2.resize(mask1, (size, size), interpolation=cv2.INTER_CUBIC)
        mask2 = cv2.resize(mask2, (size, size), interpolation=cv2.INTER_CUBIC)
        # 首先使用较大的角度进行处理
        max_angle = max(first_angleA, first_angleB)
        unioning1 = process_image_rotation_and_union(mask1, u_times, max_angle)
        clustering1 = process_image_after_clustering(unioning1, c_times, max_angle)

        unioning2 = process_image_rotation_and_union(mask2, u_times, max_angle)
        clustering2 = process_image_after_clustering(unioning2, c_times, max_angle)
        ...
        if similarity < 0.69 and min(first_angleA, first_angleB) >0.0 and first_angleA!= first_angleB:
            # 使用较小的角度重新处理
            min_angle = min(first_angleA, first_angleB)
            unioning1 = process_image_rotation_and_union(mask1, u_times, min_angle)
            clustering1 = process_image_after_clustering(unioning1, c_times, min_angle)
            unioning2 = process_image_rotation_and_union(mask2, u_times, min_angle)
            clustering2 = process_image_after_clustering(unioning2, c_times, min_angle)
            # clustering1 = extract_ellipse_region(clustering1)
            # clustering2 = extract_ellipse_region(clustering2)
            clustering1 = cv2.resize(clustering1, (size, size), interpolation=cv2.INTER_CUBIC)
            clustering2 = cv2.resize(clustering2, (size, size), interpolation=cv2.INTER_CUBIC)
            ...
            print(f"第二次最佳旋转角度：{current_best_angle}和相似度：{current_max_similarity}")
            similarity = current_max_similarity
            for angle in np.arange(current_best_angle - 0.95, current_best_angle + 1, 0.05):  # 以 0.05 度为步长
                rotation_matrix1 = cv2.getRotationMatrix2D(center, angle, 1)
                rotated_mask2 = cv2.warpAffine(clustering2, rotation_matrix1, (width, height))
        ...
        if similarity < 0.69 and first_angleA!= first_angleB:
            # mask1 用 first_angleA，mask2 用 first_angleB 处理
            ...
            print(f"第三次最佳旋转角度：{current_best_angle}和相似度：{current_max_similarity}")
            similarity = current_max_similarity
            for angle in np.arange(current_best_angle - 0.95, current_best_angle + 1, 0.05):  # 以 0.05 度为步长
                rotation_matrix1 = cv2.getRotationMatrix2D(center, angle, 1)
                rotated_mask2 = cv2.warpAffine(clustering2, rotation_matrix1, (width, height))
        ...
        resultA = calculate_spoke_number5(clustering1.copy())
        resultB = calculate_spoke_number5(clustering2.copy())
        ...
        return clustering1, clustering2, resultA, resultB, similarity, transform1, transform2,u_times, c_times

    except Exception as e:
        print(f"计算相似度时出错: {e}")
        return None, None, None, None, None

def calculate_similarity2(image_path1, image_path2):
    """
    计算两张图像的加权相似度，包括光照补偿、掩码生成、旋转处理和相似度计算。
    ...
    try:
        ...
        def calculate_dice_similarity(img1, img2):
        ...
        for angle in range(80, 202, 1):  # 以 1 度为步长旋转
            ...
            # 使用 Dice 系数计算相似度
            similarity = calculate_dice_similarity(clustering1, rotated_mask2)
            if similarity > current_max_similarity:
                current_max_similarity = similarity
                current_best_angle = angle
        ...
        for angle in np.arange(current_best_angle - 0.95, current_best_angle + 1, 0.05):  # 以 0.05 度为步长
            ...
            # 使用 Dice 系数计算相似度
            current_similarity = calculate_dice_similarity(clustering1, rotated_mask2)
            if similarity < current_similarity:
                similarity = current_similarity
...
for pair in image_pairs:
    ...
    opening_mask1, opening_mask2, spoke_number1, spoke_number2, weighted_similarity, transform1, transform2 ,u_times, c_times= calculate_similarity(
    ...
    with open('xie14output.txt', 'a', encoding='utf-8') as file:
        file.write(
