...
def find_best_similarity(mask1, mask2, first_angleA, first_angleB):
    max_similarity = -1
    best_angle = 0
    best_intersection_mask1 = None
    best_intersection_mask2 = None
    union = 0
    intersection = 0
    union_times_list = [0]
    intersection_times_list = [5]

    for union_times in union_times_list:
        break_out = False  # 用于判断是否跳出外层循环的标志变量
        for intersection_times in intersection_times_list:
            # 对掩码进行旋转和并集操作
            union_mask1 = process_image_rotation_and_union(mask1, union_times, first_angleA)
            union_mask2 = process_image_rotation_and_union(mask2, union_times, first_angleB)

            intersection_mask1 = process_image_after_clustering(union_mask1, intersection_times, first_angleA)
            intersection_mask2 = process_image_after_clustering(union_mask2, intersection_times, first_angleB)
            ...
            for angle in range(80, 202, 1):  # 以 1 度为步长旋转
                ...
                # 使用模板匹配计算相似度
                result = cv2.matchTemplate(intersection_mask1, rotated_mask2, cv2.TM_CCOEFF_NORMED)
            ...
            for angle in np.arange(current_best_angle - 0.95, current_best_angle + 1, 0.05):  # 以 0.05 度为步长
                ...
                rotated_mask2 = cv2.warpAffine(intersection_mask2, rotation_matrix1, (width, height))
            ...
            if current_max_similarity > max_similarity:
                ...
                best_intersection_mask1 = intersection_mask1
                best_intersection_mask2 = intersection_mask2

                union = union_times
                intersection = intersection_times

                if current_max_similarity > 0.90:  # 添加的判断语句
                    break_out = True  # 设置标志变量为True
                    break  # 跳出内层循环

        if break_out:  # 根据标志变量判断是否跳出外层循环
            break

    return best_intersection_mask1, best_intersection_mask2, max_similarity, union, intersection


def calculate_spoke_number2(mask):
    """
    计算图像的辐条数
...
def calculate_spoke_number1(mask):
    """
    计算图像的辐条数
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        clustering1 = process_image_after_clustering(mask1, 3, first_angleA)

        first_angleB = 360 / resultB

        print(resultB)
        clustering2 = process_image_after_clustering(mask2, 3, first_angleB)

        resultA = calculate_spoke_number3(clustering1)
        resultB = calculate_spoke_number3(clustering2)
        if resultA == resultB:
            refit = True
            return refit, clustering1, clustering2, 0,resultA,resultB
        max_angle = max(first_angleA, first_angleB)

        # 并集次数不超过4次，交集次数不超过5次，计算12种组合
        # union_times_list = [1, 2, 3, 4]
        # intersection_times_list = [1, 2, 3, 4]
        #
        # union = 0
        # intersection = 0
        #
        # best_intersection_mask1 = mask1.copy()
        # best_intersection_mask2 = mask2.copy()
        #
        # for union_times in union_times_list:
        #     break_out = False  # 用于判断是否跳出外层循环的标志变量
        #     for intersection_times in intersection_times_list:
        ...
        #         break
        #
        best_intersection_mask1, best_intersection_mask2, max_similarity, union, intersection = find_best_similarity(
            mask1, mask2, max_angle, max_angle)
        if max_similarity < 0.82:
            best_intersection_mask2A, best_intersection_mask1A, max_similarityA, unionA, intersectionA = find_best_similarity(
                mask2, mask1, max_angle, max_angle)
            if max_similarityA > max_similarity:
                max_similarity = max_similarityA
                print(f"相似度：{max_similarityA}")
                best_intersection_mask1 = best_intersection_mask1A
                best_intersection_mask2 = best_intersection_mask2A
                union = unionA
                intersection = intersectionA

        print(f"辐条数：{resultA}、{resultB}\t相似度：{max_similarity}\t 并集次数：{union}\t交集次数：{intersection}")
        # 计算加权后的相似度
        weighted_similarity = max_similarity
        with open('diffoutput5.txt', 'a', encoding='utf-8') as file:
            file.write(
                f"{image_path1[11:15]},{image_path2[11:15]},{resultA},{resultB},{max_similarity},{union},{intersection}\n")

        return refit, best_intersection_mask1, best_intersection_mask2, max_similarity,resultA,resultB
...
for pair in image_pairs:
    ...
    refit, opening_mask1, opening_mask2, weighted_similarity = calculate_similarity(
