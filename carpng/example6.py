...
def calculate_spoke_number4(mask):
    """
    计算图像的辐条数
...
def generate_mask(corrected_image):
    print("开始生成掩码")
    # img = resized = cv.resize(corrected_image, (0, 0), fx=20, fy=20)
    # # img=cv.resize(img, (100,100))
    # height, width, channels = img.shape
    # plt.figure(figsize=(10, 10))
    # size = 4
    # subplot = plt.subplot(1, size, 1)
    # plt.imshow(img)
    # subplot.set_title('Original Image')
    # gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    # binarized = cv.adaptiveThreshold(gray, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, height // 2 * 2 + 1,
    #                                  -20)
    #
    # canny = cv.Canny(gray, 250, 250)
    # subplot = plt.subplot(1, size, 2)
    # plt.imshow(canny, cmap='gray')
    # subplot.set_title('Canny Edge Detection')
    #
    # subplot = plt.subplot(1, size, 3)
    # plt.imshow(binarized, cmap='gray')
    # subplot.set_title('Binarized Image')
    #
    # binarized1 = cv.resize(binarized, (150, 150))
    # kernel = np.ones((2, 2), np.uint8)
    # binarized1 = cv.morphologyEx(binarized1, cv.MORPH_OPEN, kernel)
    # subplot = plt.subplot(1, size, 4)
    # plt.imshow(binarized1, cmap='gray')
    # subplot.set_title('Binarized Image After Opening and Closing')
    # plt.show()
    # print(binarized1.__class__)
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        resultA = calculate_spoke_number4(mask1.copy())
        print(resultA, "个辐条")


        first_angleA = 360 / resultA
        resultB = calculate_spoke_number4(mask2.copy())
        print(resultB, "个辐条")
