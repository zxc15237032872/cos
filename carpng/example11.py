...
def extract_ellipse_region(mask):
    ...
    if contours:
        ...
        print("未找到有效的轮廓，请调整参数或检查图像。")
        return mask
def calculate_similarity1(image_path1, image_path2):
    """
    ...
    try:
        ...
        return clustering1, clustering2, resultA1, resultB1, similarity,transform1,transform2
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        print(f"计算相似度时出错: {e}")
        return None, None, None, None, None
def calculate_similaritya(image_path1, image_path2):
    """
    计算两张图像的加权相似度，包括光照补偿、掩码生成、旋转处理和相似度计算。
    ...
    try:
        ...
        unioning2 = process_image_rotation_and_union(mask2, u_times, max_angle)
        clustering2 = process_image_after_clustering(unioning2, c_times, max_angle)
        cv2.imshow("clustering1", clustering1)
        cv2.waitKey(0)

        cv2.imwrite("clustering1.png", clustering1)
        cv2.imwrite("clustering2.png", clustering2)
...
image_path1 = "xie6output\\" + image_path1
image_path2 = "xie6output\\" + image_path2
...
weighted_similaritya=weighted_similarity
if weighted_similarity <0.69:
    opening_mask1a, opening_mask2a, spoke_number1a, spoke_number2a, weighted_similaritya, transform1, transform2, g_mask1, g_mask2, u_times, c_times = calculate_similaritya(
        image_path1, image_path2)

if weighted_similaritya>weighted_similarity:
    weighted_similarity=weighted_similaritya
    opening_mask1=opening_mask1a
    opening_mask2=opening_mask2a
    spoke_number1=spoke_number1a
    spoke_number2=spoke_number2a
...
cv2.imwrite("clustering3.png", opening_mask1)
cv2.imwrite("clustering4.png", opening_mask2)
# 显示图像和对应的 mask
...
plt.title(image_path1 + '的轮廓图')
plt.axis('off')

plt.subplot(4, 2, 4)
plt.imshow(transform2, cmap='gray')
plt.title(image_path2 + '的轮廓图')
plt.axis('off')
plt.subplot(4, 2, 5)
plt.imshow(g_mask1, cmap='gray')
plt.title(image_path1 + '的生成的Mask')
plt.axis('off')

plt.subplot(4, 2, 6)
plt.imshow(g_mask2, cmap='gray')
plt.title(image_path2 + '的生成的Mask')
plt.axis('off')
