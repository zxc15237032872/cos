...
def fourier_descriptor_similarity(image1, image2):
    """
    计算图像的傅里叶描述子相似度
    :param image1: 图像1
    :param image2: 图像2
    :return: 相似度得分
    """
    contours1, _ = cv2.findContours(image1.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours2, _ = cv2.findContours(image2.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # 将轮廓点转换为复数形式
    c1 = contour1[:, 0] + 1j * contour1[:, 1]
    c2 = contour2[:, 0] + 1j * contour2[:, 1]

    # 计算傅里叶变换
    f1 = np.fft.fft(c1)
    f2 = np.fft.fft(c2)

    # 取低频部分的傅里叶系数（可以根据需要调整取的系数数量）
    low_freq_num = min(len(f1), len(f2)) // 10
    f1_low = f1[:low_freq_num]
    f2_low = f2[:low_freq_num]

    # 计算傅里叶描述子的相似度（这里用相关系数衡量）
    similarity = np.corrcoef(np.real(f1_low), np.real(f2_low))[0, 1]
    similarity += np.corrcoef(np.imag(f1_low), np.imag(f2_low))[0, 1]
    similarity /= 2
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算傅里叶描述子相似度
    fd_sim = fourier_descriptor_similarity(opening_mask1, best_rotated_mask2)
    print('Fourier descriptor similarity:', fd_sim)
    # 综合考虑 SSIM 和傅里叶描述子相似度
    final_score = 0.6 * max_similarity + 0.4 * fd_sim
