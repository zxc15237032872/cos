...
def remove_insignificant_structures(mask, min_area_threshold=40, kernel_size=(1, 1)):
    # 腐蚀操作，缩小区域
...
def calculate_similarity(image_path1, image_path2):
    ...
    for angle in range(0, 360, 1):  # 以 1 度为步长旋转
        ...
        rotated_contours2, _ = cv2.findContours(rotated_mask2.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # 这里假设每个图像只有一个主要轮廓，取最大的轮廓
        if contours1 and rotated_contours2:
            contour1 = max(contours1, key=cv2.contourArea)
            contour2 = max(rotated_contours2, key=cv2.contourArea)
            difference = cv2.matchShapes(contour1, contour2, cv2.CONTOURS_MATCH_I1, 0)
            if difference < min_difference:
                min_difference = difference
                best_angle = angle
                best_rotated_mask2 = rotated_mask2

    # 为了和之前的输出格式统一，将差值转换为类似相似度的形式，差值越小相似度越高
    similarity = 1 - min_difference if min_difference < 1 else 0
    return similarity, best_angle, contour_image1, contour_image2, best_rotated_mask2
