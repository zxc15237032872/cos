...
def distance_transform_similarity(image1, image2):
    """
    计算图像的距离变换相似度
    :param image1: 图像1
    :param image2: 图像2
    :return: 相似度得分
    """
    dist1 = cv2.distanceTransform(image1.astype(np.uint8), cv2.DIST_L2, 5)
    dist2 = cv2.distanceTransform(image2.astype(np.uint8), cv2.DIST_L2, 5)

    # 归一化距离变换结果
    dist1 = cv2.normalize(dist1, None, 0, 1.0, cv2.NORM_MINMAX)
    dist2 = cv2.normalize(dist2, None, 0, 1.0, cv2.NORM_MINMAX)

    # 计算均方误差（MSE）来衡量相似度，MSE越小，相似度越高
    mse = np.mean((dist1 - dist2) ** 2)
    similarity = 1 / (1 + mse)  # 将MSE转换为相似度，范围在0到1之间
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算距离变换相似度
    dt_sim = distance_transform_similarity(opening_mask1, best_rotated_mask2)
    print('Distance Transform Similarity:', dt_sim)
    # 综合考虑 SSIM 和距离变换相似度
    final_score = 0.6 * max_similarity + 0.4 * dt_sim
