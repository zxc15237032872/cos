...
def remove_non_symmetric_contours(mask, rotation_step=60, match_threshold=0.2):
    ...
    for contour in contours:
        ...
        for angle in range(rotation_step, 360, rotation_step):
            ...
            rotated_contours, _ = cv2.findContours(rotated_mask.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            for rotated_contour in rotated_contours:
                match = cv2.matchShapes(contour, rotated_contour, cv2.CONTOURS_MATCH_I1, 0)
            ...
            if is_symmetric:
                break
        if is_symmetric:
            symmetric_contours.append(contour)

    new_mask = np.zeros_like(mask)
    cv2.drawContours(new_mask, symmetric_contours, -1, 255, -1)
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 删除非对称轮廓
    mask1 = remove_non_symmetric_contours(mask1)
    mask2 = remove_non_symmetric_contours(mask2)
