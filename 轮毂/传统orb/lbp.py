...
def lbp_similarity(image1, image2, radius=1, neighbors=8):
    """
    计算图像的局部二进制模式（LBP）相似度
    :param image1: 图像1
    :param image2: 图像2
    :param radius: LBP 算子的半径
    :param neighbors: LBP 算子的邻域点数
    :return: 相似度得分
    """
    lbp1 = texture.createLBPHFaceRecognizer().compute(image1, radius, neighbors)
    lbp2 =  texture.createLBPHFaceRecognizer().compute(image2, radius, neighbors)
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算 LBP 相似度
    lbp_sim = lbp_similarity(opening_mask1, best_rotated_mask2)

    # 综合考虑 SSIM 和 LBP 相似度
    final_score = 0.6 * max_similarity + 0.4 * lbp_sim
