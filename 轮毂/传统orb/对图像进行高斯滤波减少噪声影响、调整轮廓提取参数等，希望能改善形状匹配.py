...
def calculate_similarity(image_path1, image_path2):
    ...
    # 形状匹配（使用Hu矩）
    contours1, _ = cv2.findContours(opening_mask1.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours2, _ = cv2.findContours(best_rotated_mask2.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if len(contours1) > 0 and len(contours2) > 0:
        ...
        # 这里可以尝试调整匹配方法，如使用CONTOURS_MATCH_I2或CONTOURS_MATCH_I3
        shape_match_score = cv2.matchShapes(contours1[0], contours2[0], cv2.CONTOURS_MATCH_I1, 0)
    else:
        shape_match_score = 0

    return max_similarity, best_angle, opening_mask1, opening_mask2, best_rotated_mask2, shape_match_score
...
for pair in image_pairs:
    ...
    # 计算相似度、最佳旋转角度，获取掩码和旋转后的掩码以及形状匹配分数
    similarity, best_angle, opening_mask1, opening_mask2, rotated_mask2, shape_match_score = calculate_similarity(
        image_path1, image_path2)

    print(f"图片 {image_path1} 和 {image_path2} 的 SSIM 相似度为: {similarity}")
    print(f"最佳旋转角度为: {best_angle} 度")
    print(f"图片 {image_path1} 和 {image_path2} 的形状匹配分数为: {shape_match_score}")
