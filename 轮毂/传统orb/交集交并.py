...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算交并比
    intersection = np.logical_and(opening_mask1.astype(bool), best_rotated_mask2.astype(bool))
    union = np.logical_or(opening_mask1.astype(bool), best_rotated_mask2.astype(bool))
    iou_score = np.sum(intersection) / np.sum(union) if np.sum(union) > 0 else 0

    return max_similarity, best_angle, opening_mask1, opening_mask2, best_rotated_mask2, iou_score
...
for pair in image_pairs:
    ...
    # 计算相似度、最佳旋转角度，获取掩码和旋转后的掩码以及交并比分数
    similarity, best_angle, opening_mask1, opening_mask2, rotated_mask2, iou_score = calculate_similarity(
        image_path1, image_path2)

    print(f"图片 {image_path1} 和 {image_path2} 的 SSIM 相似度为: {similarity}")
    print(f"最佳旋转角度为: {best_angle} 度")
    print(f"图片 {image_path1} 和 {image_path2} 的交并比分数为: {iou_score}")
