...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        if result1 is not None and result2 is not None:
            if result1 == result2:
                print(f"图片 {image_path1} 和 {image_path2} 的轮辐数相同为: {result1}")
            else:
                print(f"图片 {image_path1}为 {result1}  和 {image_path2}为 {result2} ，轮辐数不同")
        else:
            if result1 is None:
                print(f"图片 {image_path1} 没有找到轮毂")
            else:
                print(f"图片 {image_path2} 没有找到轮毂")
        # 考虑旋转对称，尝试不同旋转角度
        ...
        for angle in range(0, 182, 1):  # 以 1 度为步长旋转
