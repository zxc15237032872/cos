...
def gradient_histogram_similarity(image1, image2, bins=32):
    """
    计算图像梯度直方图的相似度
    :param image1: 图像1
    :param image2: 图像2
    ...
    grad_angle1 = np.arctan2(grad_y1, grad_x1)
    ...
    grad_angle2 = np.arctan2(grad_y2, grad_x2)

    # 计算梯度幅值和方向的直方图
    hist_mag1, _ = np.histogram(grad_mag1.ravel(), bins=bins, range=(0, np.max(grad_mag1)))
    hist_angle1, _ = np.histogram(grad_angle1.ravel(), bins=bins, range=(-np.pi, np.pi))
    hist_mag2, _ = np.histogram(grad_mag2.ravel(), bins=bins, range=(0, np.max(grad_mag2)))
    hist_angle2, _ = np.histogram(grad_angle2.ravel(), bins=bins, range=(-np.pi, np.pi))

    # 归一化直方图
    hist_mag1 = hist_mag1 / np.sum(hist_mag1)
    hist_angle1 = hist_angle1 / np.sum(hist_angle1)
    hist_mag2 = hist_mag2 / np.sum(hist_mag2)
    hist_angle2 = hist_angle2 / np.sum(hist_angle2)

    # 计算直方图的相关系数作为相似度得分
    mag_similarity = np.corrcoef(hist_mag1, hist_mag2)[0, 1]
    angle_similarity = np.corrcoef(hist_angle1, hist_angle2)[0, 1]

    # 综合幅值和方向的相似度
    return (mag_similarity + angle_similarity) / 2
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算梯度直方图相似度
    gradient_similarity = gradient_histogram_similarity(opening_mask1, best_rotated_mask2)
    print(f"梯度直方图相似度为: {gradient_similarity}")
    # 综合考虑 SSIM 和梯度直方图相似度
    final_score = 0.6 * max_similarity + 0.4 * gradient_similarity
