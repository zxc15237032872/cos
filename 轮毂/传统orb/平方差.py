...
def average_hash(image, hash_size=8):
    ...
    avg = np.mean(resized)
    hash_val = np.where(resized > avg, 1, 0)
...
def calculate_similarity(image_path1, image_path2):
    ...
    # 计算平均哈希值
    hash1 = average_hash(opening_mask1)
    hash2 = average_hash(best_rotated_mask2)
    # 计算汉明距离并转换为相似度分数（汉明距离越小，相似度越高）
    hash_similarity = 1 - (hamming_distance(hash1, hash2) / float(len(hash1)))
    print("哈希相似度:")
    print(hash_similarity)
    # 综合考虑 SSIM 和哈希相似度
    final_score = 0.7 * max_similarity + 0.3 * hash_similarity
