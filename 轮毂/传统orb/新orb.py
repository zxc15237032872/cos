...
def calculate_similarity(image_path1, image_path2):
    ...
    # 使用ORB算法进行特征点匹配
    orb = cv2.ORB_create()
    kp1, des1 = orb.detectAndCompute(opening_mask1, None)
    ...
    num_good_matches = len([m for m in matches if m.distance < 500])  # 设定距离阈值筛选好的匹配点
    feature_similarity = num_good_matches / max(len(kp1), len(kp2)) if max(len(kp1), len(kp2)) > 0 else 0
    print(f"特征点匹配相似度为: {feature_similarity}")
    # 综合考虑 SSIM 和特征点匹配相似度
    final_score = 0.6 * max_similarity + 0.4 * feature_similarity
