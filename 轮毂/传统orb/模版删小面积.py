...
def remove_small_contours(image, min_area):
    """
    移除面积小于指定值的轮廓
...
def calculate_similarity(image_path1, image_path2):
    ...
    try:
        ...
        opening_mask2, rotated_mask2, angle2 = process_image_after_clustering(opening_mask2)
        opening_mask2, rotated_mask2, angle2 = process_image_after_clustering(opening_mask2)
        opening_mask1 = remove_small_contours(opening_mask1, 100)  # 这里设置最小面积为 100，可根据实际情况调整
        opening_mask2 = remove_small_contours(opening_mask2, 100)

        # 考虑旋转对称，尝试不同旋转角度
        height, width = opening_mask2.shape
