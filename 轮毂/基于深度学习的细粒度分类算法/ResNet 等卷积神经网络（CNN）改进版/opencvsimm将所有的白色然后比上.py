...
def find_two_main_colors(image):
    """
    使用 K-Means 聚类找到图像中的两种主要颜色，将接近银白色的设为红色，黑色保持黑色
    """
    img = np.array(image)
    ...
    # 计算两个聚类中心的平均亮度
    brightness1 = np.mean(center1)
    brightness2 = np.mean(center2)

    # 确定哪个聚类中心更接近银白色和黑色
    if brightness1 > brightness2:
        silver_center = center1
        black_center = center2
    else:
        silver_center = center2
        black_center = center1

    new_image = np.zeros_like(img)
    for i, label in enumerate(labels):
        if label == 0:
            if np.array_equal(kmeans.cluster_centers_[0], silver_center):
                new_image[i // img.shape[1], i % img.shape[1]] = [255, 0, 0]  # 接近银白色的设为红色
            else:
                new_image[i // img.shape[1], i % img.shape[1]] = [0, 0, 0]  # 黑色
        else:
            if np.array_equal(kmeans.cluster_centers_[1], silver_center):
                new_image[i // img.shape[1], i % img.shape[1]] = [255, 0, 0]  # 接近银白色的设为红色
            else:
                new_image[i // img.shape[1], i % img.shape[1]] = [0, 0, 0]  # 黑色
...
def similarity(feature1, feature2):
    """
    计算参照图中红色区域与旋转图中红色区域的交集与参照图中红色区域的比值
    """
    ...
    # 计算参照图中红色区域的总数
    total_red_in_ref = red_area1.sum()

    if total_red_in_ref == 0:
        return 0
    return intersection / total_red_in_ref
