...
def similarity(feature1, feature2):
    ...
    # 找到黑色区域
    black_area1 = np.all(feature1 == [0, 0, 0], axis=2)
    black_area2 = np.all(feature2 == [0, 0, 0], axis=2)

    # 计算交集
    intersection = np.logical_and(black_area1, black_area2).sum()

    # 计算参照图中黑色区域的总数
    total_black_in_ref = black_area1.sum()

    if total_black_in_ref == 0:
        return 0
    return intersection / total_black_in_ref
