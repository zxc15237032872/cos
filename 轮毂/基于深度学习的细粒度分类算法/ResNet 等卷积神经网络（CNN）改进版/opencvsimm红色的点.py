...
def similarity(feature1, feature2):
    ...
    total_red_points = 0
    matching_red_points = 0

    for y in range(size):
        for x in range(size):
            is_red1 = np.all(feature1[y, x] == [255, 0, 0])
            is_red2 = np.all(feature2[y, x] == [255, 0, 0])
            if is_red1 or is_red2:
                total_red_points += 1
                if is_red1 and is_red2:
                    matching_red_points += 1

    if total_red_points == 0:
        return 0
    return matching_red_points / total_red_points
...
axes[2, 0].imshow(np.array(Image.fromarray(feature1.reshape(colored_image1.size[1], colored_image1.size[0], 3))),
                  cmap='gray')
axes[2, 0].set_title(f"{image_path1} 特征可视化")
axes[2, 1].imshow(np.array(Image.fromarray(feature2.reshape(colored_image2.size[1], colored_image2.size[0], 3))),
                  cmap='gray')
axes[2, 1].set_title(f"{image_path2} 特征可视化")
axes[2, 2].imshow(np.array(
    Image.fromarray(feature2_rotated.reshape(colored_rotated_image2.size[1], colored_rotated_image2.size[0], 3))),
                  cmap='gray')
