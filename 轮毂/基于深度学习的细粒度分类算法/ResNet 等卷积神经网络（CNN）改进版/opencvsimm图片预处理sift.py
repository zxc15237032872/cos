...
def preprocess_image(image):
    ...
    # 高斯模糊
    img = cv2.GaussianBlur(img, (3, 3), 0)  # 减小高斯核大小
    # 自适应直方图均衡化
...
def extract_sift_features(image):
    img = np.array(image.convert('L'))
    # 调整 SIFT 参数
    sift = cv2.SIFT_create(nfeatures=500, nOctaveLayers=3, contrastThreshold=0.03, edgeThreshold=10, sigma=1.6)
...
def cosine_similarity(feature1, feature2):
    ...
    print(f"共找到 {len(good_matches)} 个匹配点")
...
# 图像预处理增强
square_image1 = preprocess_image(square_image1)

square_image2 = preprocess_image(square_image2)
best_rotated_image2 = preprocess_image(best_rotated_image2)
